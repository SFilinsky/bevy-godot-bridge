...
#[proc_macro_derive(ExportToGodot, attributes(godot_expose, expose))]
pub fn export_component(input: ::proc_macro::TokenStream) -> ::proc_macro::TokenStream {
    ...
    let fields_iter: &::syn::punctuated::Punctuated<::syn::Field, ::syn::token::Comma> = match &item.fields {
        ...
        _ => {
            return ::syn::Error::new(
                ...
                "expose only supports structs with named fields",
            )
                .to_compile_error()
                .into();
        }
